...
// ─── Constant Data ───────────────────────────────────────────────────────────
const OSI_LAYERS = [
  { num: 7, name: "Application", color: "#E74C3C", pdu: "Data", protocols: ["HTTP","HTTPS","FTP","SFTP","SSH","DNS","DHCP","SMTP","SNMP","LDAP","RDP","SIP","Telnet","NTP","PTP"], devices: ["Proxy","Load Balancer","WAF","IDS/IPS"], desc: "End-user services & APIs. Where applications access network services.", detail: "Provides network services directly to end-user applications. Identifies communication partners, determines resource availability, and synchronizes communication. Protocols here define how applications talk over the network." },
  { num: 6, name: "Presentation", color: "#E67E22", pdu: "Data", protocols: ["SSL/TLS","JPEG","GIF","MPEG","ASCII","EBCDIC"], devices: [], desc: "Data formatting, encryption, compression.", detail: "Translates data between the application and network format. Handles encryption/decryption (SSL/TLS), compression, and character encoding. Ensures data from the application layer of one system can be read by the application layer of another." },
  { num: 5, name: "Session", color: "#F1C40F", pdu: "Data", protocols: ["NetBIOS","PPTP","RPC","SIP","SAP"], devices: [], desc: "Establishes, manages, and terminates sessions.", detail: "Controls dialogues (connections) between computers. Establishes, manages, and terminates connections between local and remote application. Manages session checkpointing and recovery. Full-duplex, half-duplex, and simplex communication." },
  { num: 4, name: "Transport", color: "#2ECC71", pdu: "Segment / Datagram", protocols: ["TCP (port-based)","UDP (port-based)"], devices: ["Firewall","Load Balancer"], desc: "End-to-end delivery, flow control, error recovery.", detail: "Provides reliable (TCP) or unreliable (UDP) end-to-end data transfer. TCP uses 3-way handshake (SYN, SYN-ACK, ACK), flow control (windowing), and error correction. UDP is connectionless — faster but no guaranteed delivery. Segmentation and reassembly of data." },
  { num: 3, name: "Network", color: "#3498DB", pdu: "Packet", protocols: ["IP (v4/v6)","ICMP","IGMP","IPSec","OSPF","BGP","EIGRP","RIP"], devices: ["Router","L3 Switch","Firewall"], desc: "Logical addressing (IP), routing, path determination.", detail: "Handles logical addressing (IP addresses) and routing. Routers operate here to forward packets between networks. Determines the best path via routing protocols (OSPF, BGP, EIGRP). Fragmentation and reassembly of packets when needed." },
  { num: 2, name: "Data Link", color: "#9B59B6", pdu: "Frame", protocols: ["Ethernet (802.3)","Wi-Fi (802.11)","PPP","ARP","STP","LLDP","CDP"], devices: ["Switch","Bridge","WAP","NIC"], desc: "Physical addressing (MAC), framing, error detection.", detail: "Divided into LLC (Logical Link Control) and MAC (Media Access Control) sublayers. Uses MAC addresses for local delivery. Switches build MAC address tables. Handles frame creation, error detection (FCS/CRC), and flow control. Spanning Tree Protocol (STP) prevents loops." },
  { num: 1, name: "Physical", color: "#1ABC9C", pdu: "Bits", protocols: ["Ethernet physical","RS-232","DSL","ISDN","802.11 radio"], devices: ["Hub","Repeater","Media Converter","Modem","Cables"], desc: "Bit transmission over physical media (electrical, optical, radio).", detail: "Defines electrical, mechanical, and functional specifications. Deals with voltages, pin layouts, cabling, radio frequencies. Converts bits to signals (electrical, optical, or RF). Includes cable types: Cat5e/6/6a/7/8, fiber optic (SMF/MMF), coax." }
];

const APPLIANCES = [
  { name: "Router", layer: "3", icon: "🔀", desc: "Forwards packets between networks using IP addresses. Makes routing decisions based on routing tables populated by static routes or dynamic protocols (OSPF, BGP, EIGRP).", features: ["NAT/PAT","ACLs","Inter-VLAN routing","Subinterfaces","Policy-based routing","FHRP (HSRP/VRRP/GLBP)"] },
  { name: "Switch (L2)", layer: "2", icon: "🔲", desc: "Forwards frames based on MAC address table (CAM table). Creates separate collision domains per port. All ports are in the same broadcast domain unless VLANs are configured.", features: ["VLANs","Trunk ports (802.1Q)","STP/RSTP","Port security","PoE (802.3af/at/bt)","LLDP/CDP","Jumbo frames (9000+ MTU)"] },
  { name: "Switch (L3)", layer: "2-3", icon: "🔳", desc: "Combines switching and routing. Can route between VLANs using SVIs (Switch Virtual Interfaces) without an external router.", features: ["Inter-VLAN routing","SVIs","Static & dynamic routing","ACLs","QoS","DHCP relay"] },
  { name: "Firewall", layer: "3-7", icon: "🛡️", desc: "Filters traffic based on rules. Stateful firewalls track connection state. Next-gen firewalls (NGFW) inspect at L7 for application awareness.", features: ["Stateful inspection","ACLs","NAT","VPN termination","Zone-based policies","URL/content filtering","Trusted/untrusted zones","Screened subnet (DMZ)"] },
  { name: "IDS/IPS", layer: "3-7", icon: "🔍", desc: "IDS (Intrusion Detection System) monitors & alerts. IPS (Intrusion Prevention System) monitors & blocks. Can be signature-based or anomaly-based.", features: ["Signature-based detection","Anomaly-based detection","Inline (IPS) vs passive (IDS)","Log generation","SNMP alerts","Integration with SIEM"] },
  { name: "Load Balancer", layer: "4-7", icon: "⚖️", desc: "Distributes traffic across servers for availability and performance. Uses VIPs (Virtual IPs). Algorithms: round-robin, least connections, weighted, IP hash.", features: ["VIP (Virtual IP)","Health checks","SSL offloading","Session persistence","Active-active / active-passive","L4 vs L7 balancing"] },
  { name: "Proxy", layer: "7", icon: "🔄", desc: "Acts as intermediary for requests. Forward proxy serves clients; reverse proxy serves servers. Caches content, provides anonymity, enforces policies.", features: ["Forward proxy","Reverse proxy","Content caching","URL filtering","SSL inspection","Authentication"] },
  { name: "NAS", layer: "N/A (storage)", icon: "💾", desc: "Network-Attached Storage — file-level storage accessible over the network using CIFS/SMB or NFS protocols. Appears as a shared folder.", features: ["File-level access","CIFS/SMB & NFS","RAID support","User authentication","Easy setup","Shared folders"] },
  { name: "SAN", layer: "N/A (storage)", icon: "🗄️", desc: "Storage Area Network — block-level storage accessed via dedicated high-speed network (Fibre Channel or iSCSI). Appears as a local disk to the server.", features: ["Block-level access","Fibre Channel / iSCSI / FCoE","High performance","LUN masking/zoning","Dedicated storage network","Used for databases & VMs"] },
  { name: "Wireless AP", layer: "1-2", icon: "📡", desc: "Wireless Access Point bridges wireless clients to the wired LAN. Operates at L1 (radio) and L2 (frames). Managed by a wireless controller in enterprise.", features: ["802.11a/b/g/n/ac/ax/be","2.4/5/6 GHz bands","SSID broadcasting","WPA2/WPA3 encryption","Antenna types (omni/directional/MIMO)","Channel selection","Guest networks","802.1X authentication"] }
];
...
// ─── Section definitions ───────────────────────────────────────────────────
const ALL_SECTIONS = [
  "OSI Model", "Appliances", "Ports & Protocols", "Traffic Types", "Cabling & Connectors",
  "Topologies", "Cloud Concepts", "IPv4 Addressing", "Routing", "Switching",
  "Wireless", "Network Services", "Monitoring", "Disaster Recovery", "Security Fundamentals",
  "Attacks & Defense", "Segmentation", "Documentation", "Lifecycle & Change Mgmt", "Troubleshooting",
  "Network Lab", "Packet Deep Dive"
];
        ...
        <div style={{ flex: "1 1 340px", minWidth: 300 }}>
          {OSI_LAYERS.map((l, i) => (
            <div key={i} onClick={() => setSel(i)} style={{
              ...
              background: sel === i ? `${l.color}18` : "transparent", borderLeft: `4px solid ${l.color}`,
              borderRadius: "0 8px 8px 0", marginBottom: 2, transition: "all 0.2s",
              border: sel === i ? `1px solid ${l.color}44` : "1px solid transparent", borderLeftWidth: 4, borderLeftColor: l.color
            }}>
        ...
        <div style={{ flex: "1 1 400px", minWidth: 300 }}>
          {layer ? (
            <Card color={layer.color} title={`Layer ${layer.num}: ${layer.name}`}>
              ...
              {layer.devices.length > 0 && <div style={{ marginTop: 12 }}>
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginBottom: 20 }}>
        {APPLIANCES.map((a, i) => (
          <Pill key={i} active={sel === i} onClick={() => setSel(i)} color="#00e5ff">{a.icon} {a.name}</Pill>
        ))}
      </div>
      {dev && (
        <Card color="#00e5ff" title={`${dev.icon} ${dev.name}`}>
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: 4 }}>
            {dev.features.map(f => <Badge key={f} color="#38bdf8">{f}</Badge>)}
          </div>
        </Card>
      )}
    </div>
...
function NetworkLabSection() {
  ...
  const addDevice = useCallback((tmpl) => {
    ...
    setNodes(prev => [...prev, {
      id, ...tmpl, x: cx, y: cy,
      mac: genMAC(),
      ip: tmpl.type === "internet" ? "WAN" : genIP(),
      vlan: 1,
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (e) => {
      ...
      setNodes(prev => prev.map(n => n.id === draggingRef.current ? {...n, x: Math.max(30, Math.min(mx, 750)), y: Math.max(30, Math.min(my, 430))} : n));
    };
  ...
  const clearAll = () => { setNodes([]); setLinks([]); setSelectedNode(null); setSelectedLink(null); setSimLog([]); setSimPackets([]); setActiveFault(null); };
  ...
  const renderCanvas = () => {
    ...
    return (
      <svg ref={svgRef} width="100%" height="100%" viewBox="0 0 780 460"
        onMouseDown={handleCanvasMouseDown}
        onTouchStart={(e) => {
          const touch = e.touches[0];
          handleCanvasMouseDown({ clientX: touch.clientX, clientY: touch.clientY, preventDefault: () => e.preventDefault() });
        }}
        style={{ background: "rgba(0,0,0,0.3)", borderRadius: 10, cursor: dragging ? "grabbing" : "default", display: "block", userSelect: "none", touchAction: "none" }}
      >
        <defs>
        ...
        {/* Grid */}
        {Array.from({length:40}).map((_,i) => <line key={`gv${i}`} x1={i*20} y1={0} x2={i*20} y2={460} stroke="rgba(255,255,255,0.02)" />)}
        {Array.from({length:24}).map((_,i) => <line key={`gh${i}`} x1={0} y1={i*20} x2={780} y2={i*20} stroke="rgba(255,255,255,0.02)" />)}
        ...
        {/* Connecting line */}
        {connecting && <text x={390} y={20} textAnchor="middle" fontSize={11} fill="#00e5ff" fontFamily="monospace">Click a device to complete the connection</text>}
      </svg>
    );
  };
  ...
  return (
    <div>
      <SectionHeader icon="🏗️" title="Network Builder & Simulator" subtitle="Build a network, wire it up, inject faults, and simulate data flow through every device." />
      ...
      {tab === "build" && (
        <>
          ...
          <div style={{ marginBottom: 12 }}>
            <div style={{ fontSize: 11, color: "#64748b", marginBottom: 6, textTransform: "uppercase", letterSpacing: 1, fontFamily: "'JetBrains Mono', monospace" }}>Add Device (click to place)</div>
      ...
      {/* Canvas */}
      <div ref={canvasRef} style={{ marginBottom: 16, border: "1px solid rgba(255,255,255,0.08)", borderRadius: 12, overflow: "hidden", position: "relative", userSelect: "none", WebkitUserSelect: "none" }}>
        {renderCanvas()}
        {nodes.length === 0 && (
          <div style={{ position: "absolute", top: "50%", left: "50%", transform: "translate(-50%,-50%)", textAlign: "center", color: "#475569", pointerEvents: "none" }}>
            ...
            <div style={{ fontSize: 14, fontFamily: "'JetBrains Mono', monospace" }}>Add devices above or load a preset to get started</div>
            <div style={{ fontSize: 11, marginTop: 6 }}>Drag to move • Select + Connect to wire • Use Simulate tab to send packets</div>
          </div>
        )}
      </div>

      {/* Properties panel + Log side by side */}
      <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
        ...
        <div style={{ flex: "1 1 280px", minWidth: 260 }}>
          {selNodeObj && (
            <Card color={selNodeObj.color} title={`${selNodeObj.icon} ${selNodeObj.label || selNodeObj.type} Properties`}>
              ...
              <InfoRow label="IP Address" value={selNodeObj.ip} />
              <InfoRow label="MAC Address" value={selNodeObj.mac} />
              <InfoRow label="Subnet Mask" value={selNodeObj.subnet} />
              <InfoRow label="Gateway" value={selNodeObj.gateway} />
              <InfoRow label="VLAN" value={selNodeObj.vlan} />
              ...
              <div style={{ marginTop: 10, display: "flex", gap: 6, flexWrap: "wrap" }}>
                ...
                <div style={{ width: "100%" }}>
                  <div style={{ display: "flex", gap: 6, marginBottom: 6 }}>
                    <input placeholder="IP Address" value={selNodeObj.ip} onChange={e => setNodes(prev => prev.map(n => n.id === selNodeObj.id ? {...n, ip: e.target.value} : n))}
                    ...
                    <select value={selNodeObj.vlan} onChange={e => setNodes(prev => prev.map(n => n.id === selNodeObj.id ? {...n, vlan: parseInt(e.target.value)} : n))}
                      ...
                      {[1,10,20,30,40,50,100].map(v => <option key={v} value={v}>VLAN {v}</option>)}
                    </select>
                  </div>
                  ...
                  <div style={{ display: "flex", gap: 6, marginTop: 6 }}>
                    <input placeholder="Subnet Mask" value={selNodeObj.subnet} onChange={e => setNodes(prev => prev.map(n => n.id === selNodeObj.id ? {...n, subnet: e.target.value} : n))}
                      style={{ flex: 1, padding: "5px 8px", borderRadius: 4, border: "1px solid rgba(255,255,255,0.1)", background: "rgba(0,0,0,0.3)", color: "#e2e8f0", fontSize: 11, fontFamily: "monospace" }} />
                    <input placeholder="Gateway" value={selNodeObj.gateway} onChange={e => setNodes(prev => prev.map(n => n.id === selNodeObj.id ? {...n, gateway: e.target.value} : n))}
                      style={{ flex: 1, padding: "5px 8px", borderRadius: 4, border: "1px solid rgba(255,255,255,0.1)", background: "rgba(0,0,0,0.3)", color: "#e2e8f0", fontSize: 11, fontFamily: "monospace" }} />
                  </div>
                </div>
              </div>
        ...
        <div style={{ flex: "2 1 400px", minWidth: 340 }}>
          {simLog.length > 0 ? (
            <div style={{ background: "rgba(0,0,0,0.4)", borderRadius: 10, padding: 14, maxHeight: 400, overflowY: "auto", border: "1px solid rgba(255,255,255,0.06)" }}>
              <div style={{ fontSize: 11, color: "#64748b", textTransform: "uppercase", letterSpacing: 1, marginBottom: 8, fontFamily: "'JetBrains Mono', monospace" }}>
            ...
            <div style={{ padding: 30, textAlign: "center", color: "#475569", fontSize: 12, fontFamily: "'JetBrains Mono', monospace" }}>
